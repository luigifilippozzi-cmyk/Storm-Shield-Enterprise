...
export class UsersService {
  ...
  async findAll(tenantId: string, query: QueryUserDto): Promise<PaginatedResult<any>> {
    ...
    const baseQuery = knex('users').where({ 'users.tenant_id': tenantId, 'users.deleted_at': null });
  ...
  async findOne(tenantId: string, id: string) {
    ...
    const roles = await knex('user_role_assignments')
      .join('roles', 'user_role_assignments.role_id', 'roles.id')
      .where('user_role_assignments.user_id', id)
  ...
  async assignRole(tenantId: string, userId: string, roleId: string) {
    ...
    const [assignment] = await knex('user_role_assignments')
      .insert({ id: generateId(), user_id: userId, role_id: roleId })
  ...
  async removeRole(tenantId: string, userId: string, roleId: string) {
    const knex = await this.tenantDb.getConnection();
    await this.findOne(tenantId, userId);
    const deleted = await knex('user_role_assignments')
      .where({ user_id: userId, role_id: roleId })
      .del();
    if (!deleted) throw new NotFoundException('Role assignment not found');
    return { deleted: true };
  }
}

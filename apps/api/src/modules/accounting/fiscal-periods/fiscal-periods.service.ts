...
export class FiscalPeriodsService {
  ...
  async findAll(tenantId: string, query: QueryFiscalPeriodDto): Promise<PaginatedResult<any>> {
    ...
    const baseQuery = this.tenantDb.table('fiscal_periods').where({ tenant_id: tenantId });
  ...
  async findOne(tenantId: string, id: string) {
    const record = await this.tenantDb.table('fiscal_periods')
  ...
  async create(tenantId: string, dto: CreateFiscalPeriodDto) {
    ...
    const overlap = await this.tenantDb.table('fiscal_periods')
  ...
  async close(tenantId: string, id: string, userId: string) {
    ...
    const draftEntries = await this.tenantDb.table('journal_entries')
  ...
  async findOpenPeriodForDate(tenantId: string, date: string) {
    return this.tenantDb.table('fiscal_periods')

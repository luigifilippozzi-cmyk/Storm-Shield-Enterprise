...
export class ConsentService {
  ...
  async create(tenantId: string, customerId: string, userId: string, dto: CreateConsentDto) {
    const customer = await this.tenantDb.table('customers')
      .where({ id: customerId, tenant_id: tenantId, deleted_at: null })
      .first();
    if (!customer) throw new NotFoundException('Customer not found');

    const [record] = await this.tenantDb.table('customer_consent_records')
      .insert({
        id: generateId(),
        tenant_id: tenantId,
  ...
  async findByCustomer(tenantId: string, customerId: string) {
    return this.tenantDb.table('customer_consent_records')
  ...
  async revoke(tenantId: string, customerId: string, consentId: string, userId: string) {
    ...
    const [updated] = await this.tenantDb.table('customer_consent_records')
      .where({ id: consentId })
      .update({
        revoked_at: new Date(),
        revoked_by: userId,

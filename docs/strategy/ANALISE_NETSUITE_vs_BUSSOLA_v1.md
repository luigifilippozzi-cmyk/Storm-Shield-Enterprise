# Análise Comparativa: NetSuite vs. Bússola SSE — v1

> **Propósito:** Confrontar as melhores práticas documentadas do NetSuite (referência de arquitetura, funcionalidade e navegabilidade) com a Bússola de Produto SSE, para identificar gaps, reforçar decisões existentes e propor ajustes de priorização.
...
> **Base NetSuite:** `docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/` — consultada via WebSearch em 2026-04-21 (allowlist bloqueou fetch direto).

---

## Sumário Executivo

Navegar a doc do NetSuite **valida a direção da Bússola** em praticamente todos os pontos estratégicos e **revela 4 padrões concretos** que o SSE pode adotar sem violar o posicionamento "simpler + cheaper + purpose-built". Não surgiu nenhuma ameaça que exija reescopo amplo.

Síntese em 6 pontos:

1. **Centers (NetSuite) = Workspaces por persona (Bússola §7)** — RF-001 segue padrão de indústria. Reduz risco de adoção. Nenhuma mudança de rumo.
2. **FSM Mobile valida Gap 2 como P1** — NetSuite entrega mobile offline nativo para field service; é a referência que o mercado conhece. Manter Gap 2 em 90–120 dias reduz risco competitivo.
...
5. **Confirmações onde SSE já é mais estratégico que NetSuite:** activation tracking instrumentado (RF-003), 1099-NEC nativo (Fase 4), onboarding wizard <30 min (RF-002), MACRS nativo (FAM). Nenhum ajuste; reforçar narrativa comercial.
6. **Reforços a Bússola §5 (onde simplificamos):** 4 áreas onde NetSuite tem features avançadas que explicitamente NÃO entram no SSE — SuiteBilling, OneWorld/multi-subsidiary, SuiteFlow (workflow designer), Dashboard drag-and-drop. Documentar como descope deliberado evita debate recorrente.
...
Áreas da doc NetSuite exploradas (conforme recorte definido na abertura desta sessão):
...
| **Mobile** | NetSuite for Mobile (iOS/Android), Offline, My Dashboard, My Approvals |
...
Cada padrão NetSuite foi classificado contra os Princípios P1–P7 da Bússola §6 e contra os filtros de §5 (Simplificamos / Herdamos / Superamos). Um padrão entra na lista de sugestões **apenas** se:
...
| Accounting Period status | Open / Closed / Locked | **[Assumption]** Open/Closed binário | NetSuite tem 3 status; SSE tem 2 conforme ADR-003 | Manter 2 status. Locked é overhead sem valor para ICP. |
...
| 1099-MISC/NEC | **NetSuite NÃO gera 1099** — usa Yearli, Sovos, Track1099 | **SSE vai gerar nativo** (planejado Fase 4) | **Diferencial vs. NetSuite** | Manter. Adicionar em §5 da Bússola como "Superamos". |
...
| Dimensão | NetSuite | SSE (hoje) | Diagnóstico | Recomendação |
|---|---|---|---|---|
| **Role-based Centers** (tabbed pages por role) | Sim — standard e customizável | RF-001 em spec (PROPOSED) | **Validação:** NetSuite faz igual — nosso approach é industry-standard, não experimento | Manter RF-001. Usar "Center" ou "Workspace" — **decisão de nomenclatura** (ver §7). |
...
| Dashboard Portlets (KPI/Meter/Scorecard/Trend) | 4 tipos de portlet nativos | Dashboard atual é página custom | NetSuite é flexível, SSE é purpose-built | **Manter simplicidade.** §5 da Bússola já simplifica self-service reporting. **Adicionar: "Portlets configuráveis pelo user ficam de fora — KPIs fixos por persona."** |
...
| Dimensão | NetSuite | SSE (hoje) | Diagnóstico |
|---|---|---|---|
...
**NetSuite:** suporta múltiplos planos por billing account, charges (recurring/one-time/usage), revenue recognition por subscription line, lifecycle Draft → Pending Activation → Active.
...
1. **§1 Posicionamento simpler + cheaper + purpose-built** — validado em cada área. NetSuite oferece features que ICP body shop não usa (OneWorld, SuiteBilling, SuiteFlow, Saved Search KPIs). Descope continua certo.
2. **§2 4 Personas primárias** — NetSuite usa Centers por role; mesmo padrão. Validação de indústria.
3. **§4 Gap 1 (Landing por persona)** — NetSuite Centers é exatamente o padrão. Reduz risco de RF-001.
4. **§4 Gap 2 (Mobile Technician)** — FSM Mobile com offline é **referência concreta do mercado**. Sem isso, SSE não compete em mobility.
5. **§4 Gap 3 (Onboarding wizard)** — **NetSuite NÃO tem wizard de tenant setup** (implementação é feita por parceiro/consultor em semanas). Wizard <30 min é diferencial forte.
6. **§4 Gap 8 (Activation tracking)** — NetSuite tem dashboards pós-implementação; não vi "activation rate" como métrica padrão. Diferencial SSE.
7. **§5 Onde simplificamos** — aumenta a lista (ver §4 deste documento).
8. **§6 Princípios P1–P7** — nenhum desafiado.
9. **§7 Arquitetura de navegação** — Centers do NetSuite = Workspaces do SSE. Mesmo padrão.
...
| Dimensão | NetSuite faz | SSE faz | Racional |
|---|---|---|---|
...
> **1099-NEC nativo** | NetSuite **não gera 1099** — depende de integradores externos (Yearli, Sovos, Track1099). SSE gera nativamente por ser essencial ao ICP (contractors 1099 são operação padrão do shop).

E adicionar:

> **MACRS nativo** | NetSuite trata MACRS como "alternate tax method" separado. SSE tem MACRS nativo em FAM (ADR-008) porque é requisito IRS para veículos do shop — feature essencial ao ICP, não extensão.
...
Racional: gap concreto identificado na análise NetSuite FSM Mobile. Body shop tem WiFi ruim tipicamente. Sem offline, mobile do técnico vira "ferramenta que só funciona no escritório" — contradiz P2.

### 4.4 Expansão de §7 — Navegação

Adicionar nota de decisão:

> **Nomenclatura:** adotamos o termo **"Workspace"** (não "Center" como NetSuite) para não importar jargão NetSuite para o SSE. Workspace é autodescritivo em EN e PT-BR.
...
|---|---|---|---|---|
| **P1** | 60–90 dias | **RF-004 Customer 360** (tela unificada) | Fecha fricção CRM — não está nos 8 gaps originais, mas é 90% dos cliques do Estimator | NetSuite tem como padrão. Reduz tempo médio para abrir estimate. |
...
**Gap fechado:** complementa Gap 5. Inspiração: NetSuite Payment Hold.
**Persona primária:** Estimator.
**Princípio:** P5.
**Prioridade:** P1, Fase 2.
...
Padrão copiado de NetSuite In-Transit Payments.

---

## 7. O que ficou fora (anti-recomendações explícitas)

Para evitar debate recorrente, documentar o que **NÃO** entra mesmo tendo sido visto no NetSuite:
...
3. **Departments / Classes / Locations / Custom Segments / Subsidiaries** — classification trio do NetSuite fica de fora; anti-target §1.
...
Estas 13 decisões de descope ficam documentadas aqui como referência rápida para discovery futuro. Próxima vez que alguém perguntar "mas e se fizermos X do NetSuite?", este documento responde.
...
documentação pública do NetSuite (docs.oracle.com/en/cloud/saas/netsuite) e
a Bússola de Produto SSE v1.0 (ADR-009). Análise documentada em
docs/strategy/ANALISE_NETSUITE_vs_BUSSOLA_v1.md.
...
| Data | Item | Autor |
|---|---|---|
| 2026-04-21 | Análise comparativa NetSuite vs Bússola — v1 | PO Assistant (Cowork) |
...
- [Table of Contents](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/toc.htm)
- [Customer Relationship Management (CRM)](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_N3945914.html)
- [Customer 360 View](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_0709124630.html)
...
- [Form 1099-MISC](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_N2045726.html)

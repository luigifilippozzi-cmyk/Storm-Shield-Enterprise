...
Referencia rapida para o Dev Manager:
...
- **Branches:** Apenas `main` (todas as stale branches foram deletadas em 2026-04-12)
- **PRs:** 17 merged, 0 open (ultimo: PR #17 post-migration cleanup)
- **CRLF fix:** `.gitattributes` com `* text=auto eol=lf` — NUNCA deve haver phantom diffs de line endings
- **Deploy Web:** Vercel — GREEN
- **Deploy API:** Fly.io — RED (secrets `FLY_API_TOKEN` e `DATABASE_URL_UNPOOLED` nao configurados no GitHub)
- **Squad:** 4 subagentes em `.claude/agents/` (security-reviewer, test-runner, db-reviewer, frontend-reviewer)
- **MCP:** GitHub + Agent Teams habilitados em `.claude/settings.json`
- **Fase 1 MVP:** ~90% concluido — 11 modulos backend, 26 paginas frontend, 16 test suites, 14 controllers, 8 migrations
- **Prioridades imediatas:** (1) configurar GitHub Secrets para Deploy API, (2) coverage report 80%, (3) ADR-007
...
- NAO referenciar caminhos do OneDrive (o repo esta em C:\Dev\storm-shield-enterprise)
